...
; ; 	ADDI 	-30            ; 18 is max value(wall), if less it will be a reflector
; ; 	JNEG 	Front          ; A reflector is seen check the front
; ; 	;CALL	IsAWall        ; a wall was seen move and rotate to adjust
; ; 	JUMP	Die
; ; 	JUMP 	Path           ; resume forward motion by jumping back to path
...
Wait75: 			   ; approximately travels for 1ft
	OUT    TIMER
Wloop75:
	IN     TIMER
	OUT    XLEDS       ; User-feedback that a pause is occurring.
	ADDI   -7         ; 4.5 second at 10Hz.
	JNEG   Wloop75
	RETURN
	
Wait5: 			   ; approximately travels for .75ft
	OUT    TIMER
Wloop5:
	IN     TIMER
	OUT    XLEDS       ; User-feedback that a pause is occurring.
	ADDI   -5         ; 4.5 second at 10Hz.
	JNEG   Wloop5
	RETURN
...
SonarFrontM:
	LOADI &B00001100 ;Subroutine for enabling Front middle sensors
	OUT SONAREN    ; numbers 2 and 3
	OUT SONARINT
	RETURN
	
	
SonarFrontR:
	LOADI &B00000110 ;Subroutine for enabling Front Right sensors
	OUT SONAREN    ; numbers 1 and 2
	OUT SONARINT
	RETURN

	
SonarFrontL:
	LOADI &B00011000  ;Subroutine for enabling Front Left sensors
	OUT SONAREN  ; numbers 3 and 4
	OUT SONARINT
	RETURN 

SonarSides:
	LOADI &B00010010  ;Subroutine for enabling Front Left sensors
	OUT SONAREN  ; numbers 1 and 4
	OUT SONARINT
	RETURN 
